 ...
 * <ul>
 *   <li>The authenticated user's ID from {@link com.bravos.recruitment.libs.starter.context.AuthContextHolder}</li>
 *   <li>The configured {@code starter.audit.data.default-system-id} when no user is authenticated</li>
 * </ul>
 *
 * <p>This configuration can be disabled by setting {@code starter.audit.data.enabled=false}</p>
 *
...
@ConditionalOnProperty(prefix = "starter.audit.data", name = "enabled", havingValue = "true", matchIfMissing = true)
